...
class LineGeometry extends UnskinnedGeometry {
  ...
  factory LineGeometry.segments(List<vm.Vector3> points) {
    if (points.length < 2 || points.length % 2 != 0) {
      throw ArgumentError('Segments mode requires even number of points (pairs)');
    }
  ...
  void _generateTraditionalGeometry() {
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      final perpendicular = vm.Vector3(-direction.y, direction.x, 0.0).normalized();
    ...
    final indexData = Uint16List.fromList(indices.map((i) => i.clamp(0, 65535)).toList());
